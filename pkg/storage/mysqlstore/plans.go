...
const planColumns = `id, plan_identifier, database_name, database_type,
	deployment, target, repository, pull_request, environment, schema_files, plan_data, head_sha, created_at`
...
func (s *planStore) Create(ctx context.Context, plan *storage.Plan) (int64, error) {
	...
	result, err := s.db.ExecContext(ctx, `
		INSERT INTO plans (plan_identifier, database_name, database_type, deployment, target, repository, pull_request, environment, schema_files, plan_data, head_sha, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, plan.PlanIdentifier, plan.Database, plan.DatabaseType, plan.Deployment, plan.Target, plan.Repository, plan.PullRequest, plan.Environment, string(schemaFilesJSON), string(planDataJSON), plan.HeadSHA, plan.CreatedAt)
	if err != nil {
